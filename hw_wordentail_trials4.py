...
# train data
print(pd.DataFrame(wordentail_data['word_disjoint']['train'])[1].value_counts())
...
def bert_concatatenate(u, v):
    ...
    return np.array([encoded_text['input_ids'], encoded_text['attention_mask'], encoded_text['token_type_ids']])
...
class HfBertClassifierModel(nn.Module):
    ...
    def forward(self, X):
        ...
        (final_hidden_states, cls_output) = self.bert(indices,
                                                      attention_mask=mask,
                                                      token_type_ids=segmentid)
        return self.W(cls_output)

class HfBertClassifier(TorchShallowNeuralClassifier):
    def define_graph(self):
        return HfBertClassifierModel(self.n_classes_)

net = HfBertClassifier(
    batch_size=16,  # Crucial; large batches will eat up all your memory!
    max_iter=5  # lets just run for 5 iterations for now.
    )

train_data = wordentail_data['word_disjoint']['train']
dev_data = wordentail_data['word_disjoint']['dev']
train_data_tiny = wordentail_data['word_disjoint']['train'][:32]
dev_data_tiny = wordentail_data['word_disjoint']['dev'][:32]
...
word_disjoint_experiment = nli.wordentail_experiment(
    ...
    vector_combo_func=bert_concatatenate)
